...
import ComponentRegistry from '@/components/eventManagement/table/ComponentRegistry';
...
const { event, column } = defineProps<{
...
watch(
  ...
  (field) => {
    ...
    if (data.source === 'form') {
      if (FormSelectCache.get(event, field) != null) {
...
const fieldOptions = computed<QSelectOption[]>(() => {
  ...
  return extractFormFields(event.form);
});
