...
import { FormSelectCache } from '@/components/eventManagement/table/tableCells/FormSelectCache';
import { useObjectTranslation } from '@/composables/objectTranslation';
import type {
  IconMappingOptions,
  IconMapping,
  IconMappingFallback,
} from '@/components/eventManagement/table/tableCells/IconMappingOptions';

const { event, field } = defineProps<TableCellOptionsProps>();
...
const valueOptions = computed<QSelectOption[] | undefined>(() => {
  ...
  const options = FormSelectCache.get(event, field);
  if (!options) {
    return undefined;
  }
