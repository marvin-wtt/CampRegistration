<template>
  ...
  >
    ...
    <calendar-navigation-bar
      ...
      :end="event.endAt"
      :current="anchorDate"
      :restrict-to-event="!settings.browseOutsideEventDates"
      :editable="canUpdate"
      :deletable="canDelete"
      :creatable="canCreate"
    ...
    >
      <div class="col relative-position">
        <div class="absolute fit">
          <q-calendar-day
            ...
            :interval-class="outOfEventIntervalClass"
      ...
      <calendar-backlog-panel
        ...
        @dragstart="(e: DragEvent, event: ProgramItem) => onDragStart(e, event)"
...
import CalendarNavigationBar from '@/components/eventManagement/programPlanner/CalendarNavigationBar.vue';
import CalendarItem from '@/components/eventManagement/programPlanner/CalendarItem.vue';
import CalendarDayItem from '@/components/eventManagement/programPlanner/CalendarDayItem.vue';
import type { DragAndDropScope } from '@/components/eventManagement/programPlanner/DragAndDropScope';
import type { ProgramPlannerSettings } from '@camp-registration/common/settings';
import ProgramItemAddDialog from '@/components/eventManagement/programPlanner/dialogs/ProgramItemAddDialog.vue';
import ProgramItemEditDialog from '@/components/eventManagement/programPlanner/dialogs/ProgramItemEditDialog.vue';
import CalendarSettingsDialog from '@/components/eventManagement/programPlanner/dialogs/CalendarSettingsDialog.vue';
import CalendarBacklogPanel from '@/components/eventManagement/programPlanner/CalendarBacklogPanel.vue';
import {
  daysBetweenDates,
  formatLocalDate,
  isoToLocalDate,
...
const canCreate = computed<boolean>(() => can('event.program_items.create'));
const canUpdate = computed<boolean>(() => can('event.program_items.update'));
const canDelete = computed<boolean>(() => can('event.program_items.delete'));
...
const emit = defineEmits<{
  (e: 'update', id: string, event: ProgramItemUpdateData): void;
  (e: 'add', event: ProgramItemCreateData): void;
  (e: 'delete', id: string): void;
}>();

const { settings } = useEventSettings<ProgramPlannerSettings>(
...
function initialRange(): number {
  const max =
    daysBetweenDates(new Date(event.startAt), new Date(event.endAt)) + 1;
...
function initialAnchorDate(): string {
  const startDate = parseLocalDate(isoToLocalDate(event.startAt));
  const endDate = parseLocalDate(isoToLocalDate(event.endAt));
...
function clampWindowStart(date: string, days: number): string {
  if (settings.browseOutsideEventDates) {
    return date;
  }

  const startMs = parseLocalDate(isoToLocalDate(event.startAt)).getTime();
  const endMs = parseLocalDate(isoToLocalDate(event.endAt)).getTime();
...
function compareForLayout(a: ProgramItem, b: ProgramItem): number {
...
const backlogEvents = computed<ProgramItem[]>(() =>
...
function spansOverlap(a: ProgramItem['plan'], b: ProgramItem['plan']): boolean {
  if (!viewBoth.value) {
    return true;
  }
...
function onDayEventAdd({ scope }: CalendarEvent) {
  ...
  quasar
    .dialog({
      component: ProgramItemAddDialog,
      componentProps: {
        date: scope.timestamp.date,
        time: null,
        duration: null,
        plan: activePlan.value,
        dateTimeMin: event.startAt,
        dateTimeMax: event.endAt,
        locales: event.locales,
      },
    })
    .onOk((programItem: ProgramItemCreateData) => {
      emit('add', programItem);
    });
}
...
const resizePreview = ref<
  ...
  | { kind: 'plan'; ids: Set<string>; plan: ProgramItem['plan'] }
  | null
>(null);

function resizedDuration(event: ProgramItem): number | undefined {
...
function resizedPlan(event: ProgramItem): ProgramItem['plan'] | undefined {
  const preview = resizePreview.value;
...
function onEventResize(event: ProgramItem, delta: number, preview: boolean) {
  if (!canUpdate.value) {
    return;
  }
...
function onEventDuplicate(programItem: ProgramItem) {
  if (!canCreate.value) {
    return;
  }
  quasar
    .dialog({
      ...
      componentProps: {
        ...
        time: programItem.time
          ? addMinutesToTime(programItem.time, 30)
          : programItem.time,
        duration: programItem.duration,
        location: programItem.location,
        details: programItem.details,
    ...
    .onOk((created: ProgramItemCreateData) => {
      emit('add', created);
    });
}

function onEventEdit(programItem: ProgramItem) {
  if (!canUpdate.value) {
    return;
  }
  quasar
    .dialog({
      component: ProgramItemEditDialog,
      componentProps: {
        event: programItem,
        dateTimeMin: event.startAt,
        dateTimeMax: event.endAt,
    ...
    .onOk((updated: ProgramItemUpdateData) => {
      emit('update', programItem.id, updated);
    });
}
...
function onScheduleFromBacklog(event: ProgramItem) {
  if (!canUpdate.value) {
    return;
  }
...
function onEventDelete(event: ProgramItem) {
  if (!canDelete.value) {
    return;
  }
  quasar
...
function swapPlan(plan: ProgramItem['plan']): ProgramItem['plan'] {
  if (plan === 'both') {
    return 'both';
  }
  return plan === 'a' ? 'b' : 'a';
...
function planSideStyle(plan: ProgramItem['plan']) {
...
function onDragStart(e: DragEvent, event: ProgramItem): void {
  if (!e.dataTransfer) {
    return;
  }
...
function computeGroupTargets(
  sourceEvent: ProgramItem,
  eventUpdate: ProgramItemUpdateData,
  groupIds: string[],
  ignoreIds: Set<string>,
  claims: PlanClaim[],
): GroupTarget[] {
...
function onDrop(
  ...
  let eventUpdate: ProgramItemUpdateData;
  switch (type) {
    case 'interval': {
...
function outOfEventIntervalClass({
  scope,
}: {
  scope: { timestamp: Timestamp };
}): Record<string, boolean> {
  const { date, time } = scope.timestamp;
  const start = localDateTimeParts(event.startAt);
  const end = localDateTimeParts(event.endAt);
  ...
  return { 'cal-outside-event': outside };
}
...
type UndoAction =
  | { type: 'update'; id: string; data: ProgramItemUpdateData }
  | { type: 'delete'; eventData: ProgramItemCreateData };
...
// no-op when browsing outside the event dates is enabled.
...
// Diagonal hatching marks hours outside the event period as unavailable;
