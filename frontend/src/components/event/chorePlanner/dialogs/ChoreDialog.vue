...
const data = reactive<ChoreCreateData>({
  ...
  excludeStaff: props.chore?.excludeStaff ?? false,
...
function onOKClick(): void {
  const payload: ChoreCreateData | ChoreUpdateData = { ...data };
...
field:
  ...
  excludeStaff:
    label: 'Exclude staff'
    hint: 'Leave staff (and staff-only rooms) out of suggestions for this chore.'
  balanceCountries:
...
field:
  ...
  excludeStaff:
    label: 'Wyklucz kadrę'
    hint: 'Pomiń kadrę (i pokoje przeznaczone wyłącznie dla kadry) w sugestiach dla tego obowiązku.'
  balanceCountries:
...
field:
  ...
  excludeStaff:
    ...
    hint: 'Nenavrhovat vedoucí (ani pokoje určené jen pro vedoucí) pro tuto povinnost.'
  balanceCountries:
    label: 'Vyvážit země'
