...
// Both units' suggestion lists are fetched once per selected chore (not per
...
const participantSuggestions = ref<ChoreAssignmentSuggestionCandidate[]>([]);
const roomSuggestions = ref<ChoreAssignmentSuggestionCandidate[]>([]);

const selectedChore = computed<Chore | undefined>(() => {
  return props.chores.find((chore) => chore.id === choreId.value);
});

const choreOptions = computed<QSelectOption[]>(() => {
  return props.chores.map((chore) => ({
    label: to(chore.name),
    value: chore.id,
  }));
});

function defaultRotationUnit(): ChoreRotationUnit {
  // Mirrors roomOptions' filtering (occupied, not staff-only per the selected
  // chore) rather than reading that computed directly — it isn't declared yet
  // at this point in setup, since its value seeds this ref's initializer.
  const chore = props.chores.find((c) => c.id === choreId.value);
  const excludeStaff = chore?.excludeStaff ?? false;
...
function participantStat(
  id: string,
): ChoreAssignmentSuggestionCandidate | undefined {
  return participantSuggestions.value.find((candidate) => candidate.id === id);
}

function roomStat(id: string): ChoreAssignmentSuggestionCandidate | undefined {
...
const participantOptions = computed<QSelectOption[]>(() => {
  const excludeStaff = selectedChore.value?.excludeStaff ?? false;
...
// How many more people the chore's usual size still calls for, given who's
...
const remainingHeadcount = computed<number | null>(() => {
  const target = selectedChore.value?.defaultCount;
  if (target == null) {
    return null;
  }
...
const suggestedCandidates = computed<ChoreAssignmentSuggestionCandidate[]>(
  () => {
    if (rotationUnit.value === 'ROOM') {
      ...
      // needed. A chore needing 4 people suggests one room of 4, or two of 2,
      // rather than a fixed chip count that ignores how big rooms actually are.
      const picked: ChoreAssignmentSuggestionCandidate[] = [];
      let covered = 0;
...
// Only the latest request's result is ever applied — switching chore again
...
watch(
  ...
  async (id) => {
    ...
    const [participantResult, roomResult] = await Promise.all([
      choreAssignmentStore.fetchSuggestions(id, 'PARTICIPANT'),
      choreAssignmentStore.fetchSuggestions(id, 'ROOM'),
    ]);
...
// Past slot labels used for this chore — e.g. Kitchen's recurring
...
const pastSlots = computed<string[]>(() => {
  ...
  for (const existing of choreAssignmentStore.data ?? []) {
    if (existing.choreId !== choreId.value || !existing.slot) {
      continue;
    }
...
function candidateChipLabel(
  candidate: ChoreAssignmentSuggestionCandidate,
): string {
...
// must not silently pull its staff occupants in when this chore excludes staff.
...
function onOKClick(): void {
  // The chore select has a required rule, so the form can't submit without it.
  if (!choreId.value) {
    return;
  }
