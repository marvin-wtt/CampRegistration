...
// Defaults to the first chore once chores load, but only until the user picks
// one themselves.
const historyChoreId = ref<string | null>(null);
watch(
  () => props.chores,
  (list) => {
    if (historyChoreId.value === null && list.length > 0) {
      historyChoreId.value = list[0]!.id;
    }
  },
  { immediate: true },
);
...
// Scoped to one chore at a time — mixing counts across unrelated chores
// (Kitchen and Trash, say) into one total isn't actionable.
const historyAssignments = computed<ChoreAssignment[]>(() => {
  return props.assignments.filter(
    (assignment) => assignment.choreId === historyChoreId.value,
  );
});
...
dutyLabel: 'Chore'
empty: 'No history yet for this chore.'
count: '{count} time(s)'
...
dutyLabel: 'Obowiązek'
empty: 'Brak historii dla tego obowiązku.'
count: '{count} razy'
...
dutyLabel: 'Povinnost'
empty: 'Zatím žádná historie pro tuto povinnost.'
count: '{count}×'
