...
import type { ContactDraft } from '@/components/eventManagement/contact/Contact';
import { useEventDetailsStore } from '@/stores/event-details-store';
import { usePermissions } from '@/composables/permissions';
import { useRealtimeCollection } from '@/composables/realtimeCollection';
import { useAPIService } from '@/services/APIService';
import ContactForm from '@/components/eventManagement/contact/ContactForm.vue';
import SentMessageHistory from '@/components/eventManagement/contact/SentMessageHistory.vue';
...
async function onResend(template: Message) {
  ...
  try {
    const attachments = template.attachments?.length
      ? await apiService.duplicateMessageAttachments(eventId, template.id)
      : [];
...
async function onDelete(template: Message) {
  ...
  try {
    await apiService.deleteMessage(eventId, template.id);
...
header:
  ...
  viewCaption: 'Review the messages sent for this event.'
empty:
