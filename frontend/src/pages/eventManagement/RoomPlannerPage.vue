    ...
    <div class="planner-content col-12 col-md-11 col-lg-10 column no-wrap">
      ...
      >
        <room-list
          ...
          :editable="can('event.rooms.edit')"
          :deletable="can('event.rooms.delete')"
          :assignable="can('event.rooms.beds.edit')"
...
import RoomList from '@/components/eventManagement/roomPlanner/RoomList.vue';
import RoomListSkeleton from '@/components/eventManagement/roomPlanner/RoomListSkeleton.vue';
import RoomOrderDialog from '@/components/eventManagement/roomPlanner/dialogs/RoomOrderDialog.vue';
import RoomCreateDialog from '@/components/eventManagement/roomPlanner/dialogs/RoomCreateDialog.vue';
import RoomEditDialog from '@/components/eventManagement/roomPlanner/dialogs/RoomEditDialog.vue';
import type {
  Room,
  RoomCreateData,
  RoomUpdateData,
...
const { settings } = useEventSettings<RoomPlannerSettings>(
...
async function createRoom(
  ...
  return withProgressNotification('create', async () => {
    const room = await apiService.createRoom(eventId, createData);
...
async function updateRoom(
  ...
  return withProgressNotification('update', async () => {
    let updatedRoom = await apiService.updateRoom(eventId, room.id, updateData);

    const diff = capacity - room.beds.length;
    if (diff > 0) {
      for (let i = 0; i < diff; i++) {
        await apiService.createBed(eventId, room.id);
      }
    } else if (diff < 0) {
      ...
      for (const bed of removableBeds) {
        await apiService.deleteBed(eventId, room.id, bed.id);
      }
    }

    if (diff !== 0) {
      updatedRoom = await apiService.fetchRoom(eventId, room.id);
    }
...
async function bulkUpdateRooms(
  ...
  return withProgressNotification('update', async () => {
    const updatedRooms = await apiService.bulkUpdateRooms(eventId, rooms);
...
async function deleteRoom(roomId: string) {
  ...
  await withProgressNotification('delete', async () => {
    await apiService.deleteRoom(eventId, roomId);
...
request:
  ...
  update:
    ...
    invalid: 'Invalid room id or event id'
  update-bed:
    error: 'Failed to update bed'
    invalid: 'Invalid bed id or event id'
  delete:
