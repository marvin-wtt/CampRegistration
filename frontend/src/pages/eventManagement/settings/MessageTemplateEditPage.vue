    ...
    >
      ...
      >
        <q-list class="q-py-xs">
          <q-expansion-item
            v-for="{ trigger, templates: triggerTemplates } in templates"
            :key="trigger"
          ...
          >
            <template #header>
              <q-item-section avatar>
                <q-icon
                  :name="TEMPLATE_ICONS[trigger] ?? 'mail_outline'"
                  color="primary"
                />
              </q-item-section>
              ...
              <q-item-section>
                <q-item-label>
                  {{ t(`template.${trigger ?? 'default'}.label`) }}
                </q-item-label>
                <q-item-label caption>
                  {{ t(`template.${trigger ?? 'default'}.description`) }}
                </q-item-label>
              </q-item-section>
            </template>
            ...
            <q-list>
              <q-item
                v-for="{ country, template } in triggerTemplates"
                :key="country"
                :clickable="!!template"
              ...
              >
                ...
                <q-item-section side>
                  <template v-if="template?.id">
                    <q-btn
                      v-if="
                        can('event.message_templates.edit') ||
                        can('event.message_templates.delete')
                      "
                    ...
                    >
                      <q-menu>
                        <q-list style="min-width: 180px">
                          <q-item
                            v-if="can('event.message_templates.edit')"
                            clickable
                            v-close-popup
                            @click="editTemplate(template)"
                          >
                          ...
                          <q-item
                            v-if="can('event.message_templates.delete')"
                            clickable
                            v-close-popup
                            class="text-negative"
                  ...
                  <template v-else>
                    <q-btn
                      v-if="can('event.message_templates.create')"
                      ...
                      dense
                      unelevated
                      @click="addTemplate(trigger, country)"
                    />
                  </template>
                </q-item-section>
              </q-item>
...
import MessageEditDialog from '@/components/eventManagement/settings/emails/MessageEditDialog.vue';
...
const loading = computed<boolean>(() => {
  return isLoading.value || eventDetailsStore.isLoading;
});

const pageError = computed<string | null>(() => {
  return error.value ?? eventDetailsStore.error;
});

async function loadData() {
  await forceFetch(async () => {
    return api.fetchMessageTemplates(queryParam('eventId'));
  });
}
...
const templates = computed<MappedTemplate[]>(() => {
  ...
  const confirmationMode = event.value?.confirmationMode;
    ...
    templates: countries.map((country) => {
      return {
        ...
        template: values.find(
          (t) => t.trigger === trigger && t.country === country,
        ),
      };
    }),
  }));
...
function addTemplate(trigger: string, country: string) {
  const event = eventDetailsStore.data;
  if (!event) {
    throw new Error('No event details loaded!');
  }

  // Search for existing template for same trigger but different country
  const template = data.value?.find((template) => template.trigger === trigger);

  quasar
    .dialog({
      component: MessageEditDialog,
      componentProps: {
        name: t(`template.${trigger}.label`),
        trigger,
        country,
        form: event.form,
        ...
        saveFn: async (message: {
          ...
          await withResultNotification('create', async () => {
            return api.createMessageTemplate(event.id, {
              trigger,
              country,
...
function editTemplate(template: MessageTemplate | undefined) {
  ...
  quasar
    .dialog({
      component: MessageEditDialog,
      componentProps: {
        name: t(`template.${template.trigger}.label`),
        ...
        saveFn: async (message: {
          ...
          await withResultNotification('update', async () => {
            return api.updateMessageTemplate(event.id, template.id, {
              subject: message.subject,
              body: message.body,
              attachmentIds: message.attachmentIds ?? undefined,
            });
...
async function deleteTemplate(id: string | undefined) {
  ...
  await withResultNotification('delete', async () => {
    return api.deleteMessageTemplate(event.id, id);
  }).then(async () => {
    await loadData();
  });
}
