<template>
  <page-state-handler
    ...
    :error
    :loading
    class="event-edit-page row justify-center"
  >
    <q-form
      v-if="event"
      greedy
        ...
        <div class="col-12 col-md-6 column q-gutter-y-md">
          ...
          >
            ...
            <q-card-section class="column q-gutter-y-md">
              <!-- Dates -->
              <date-range-input
                v-model:from="event.startAt"
                v-model:to="event.endAt"
                :disable="loading"
          ...
          >
            ...
            <q-card-section class="column q-gutter-y-md">
              <!-- Registration opens -->
              <date-time-input
                v-model="event.registrationOpensAt"
                :disable="loading"
                ...
                :rules="[
                  (val?: string | null) =>
                    ...
                    new Date(val) < new Date(event.registrationClosesAt) ||
              ...
              <!-- Registration closes -->
              <date-time-input
                v-model="event.registrationClosesAt"
                :disable="loading"
              ...
              <!-- Admission mode -->
              <q-select
                v-model="event.confirmationMode"
...
const { data, error, isLoading } = storeToRefs(eventStore);

const event = ref<EventDetails | undefined>(initialValue());
const updating = ref<boolean>(false);

onMounted(async () => {
  await Promise.allSettled([eventStore.fetchData()]);
});
...
const preventLeave = computed<boolean>(() => {
  ...
  return JSON.stringify(event.value) !== JSON.stringify(clone(data.value));
});

const confirmationModeOptions = computed<
  QSelectOption<EventDetails['confirmationMode']>[]
>(() => {
  return [
    {
...
function clone(value: EventDetails): EventDetails {
...
function initialValue(): EventDetails | undefined {
  return data.value ? clone(data.value) : undefined;
}

function onReset() {
  event.value = initialValue();
}
...
async function onSubmit() {
  updating.value = true;
  const value: Event | undefined = event.value;

  if (value === undefined) {
    return;
  }

  const newEvent = await eventStore.updateData(value);
  if (!newEvent) {
...
subtitle: 'Edit the general details of this event.'
