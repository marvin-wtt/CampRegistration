  ...
  >
    <div class="privacy-shell column col-12 col-sm-10 col-md-8 q-gutter-md">
      ...
      >
        ...
        <q-card-section>
          <!-- Nothing published is the one state a camp cannot work around,
      ...
      <!-- The camp's own additions. Every row states where it comes from, so
        ...
        <q-card-section>
          ...
          >
            ...
            <!-- Inherited entries show the organization's own answer rather
                 ...
                 camp silently override the notice it is adding to. -->
            <div
            ...
            <!-- Rendered for inherited entries too: the organization declared
                 the category, but both notes ask about this camp's own form,
                 and it is this camp's manager who builds it. -->
    ...
    <!-- The organization's notice on its own, and the same notice with this
         camp's additions folded in: the pair is the only way to see what the
         camp actually changes. -->
...
import {
  CAMP_DATA_TYPE_DATA_CATEGORIES,
...
/** The organization's published notice — what this camp is adding to. */
...
const organizationId = computed(() => camp.value?.organizationId);
const canEdit = computed(() => can('camp.edit'));
...
onMounted(async () => {
  try {
    // On a reload this page is what instantiates the camp store, so the store's
    // own route guard has already run and nothing has fetched the camp yet.
    await campDetailsStore.fetchData();

    const id = campId.value;
    if (!id) {
      error.value = campDetailsStore.error;
      return;
    }
...
/**
 * The organization's notice as a registrant would read it, without this camp's
...
const previewNotice = computed<PublishedPrivacyNotice | null>(() => {
  ...
  return {
    ...
    notice: organizationContent.value
      ...
      : null,
    campVersion: publishedVersion.value,
  };
});
...
type EntryOrigin = 'organization' | 'camp' | null;
...
function categoryOrigin(key: PrivacyDataCategoryKey): EntryOrigin {
  ...
  return hasDataCategory(key) ? 'camp' : null;
}
...
function recipientOrigin(key: PrivacyRecipientKey): EntryOrigin {
  ...
  return hasRecipient(key) ? 'camp' : null;
}
...
// rather than this camp's — hiding them leaves the list the author came for.
...
function toggleDataCategory(key: PrivacyDataCategoryKey, on: boolean) {
  ...
  content.value.dataCategories = on
    ? [
        ...entries,
        // Pre-select the Art. 9 basis nearly every camp actually relies on,
...
async function publish() {
  ...
  try {
    const context = await publishCampAddendum(id, { ...content.value });
    publishedVersion.value = context.publishedVersion;
    publishedAt.value = context.publishedAt;
