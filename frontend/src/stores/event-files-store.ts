...
export const useEventFilesStore = defineStore('eventFiles', () => {
  ...
  const {
    ...
    lazyFetch,
    backgroundFetch,
  } = useServiceHandler<ServiceFile[]>('eventFiles');
  ...
  const pendingSlots = computed<string[]>(() => {
    const form = eventStore.data?.form;
    if (!form) {
      return [];
    }
  ...
  // of the event's configured locales (only when no null-locale fallback covers all locales).
  const slotsWithMissingLocales = computed<
    { slot: string; missingLocales: string[] }[]
  >(() => {
    const locales = eventStore.data?.locales;
    if (!locales?.length) {
      return [];
    }
  ...
  async function fetchData(opts?: { background?: boolean }) {
    ...
    const fetcher = () => apiService.fetchEventFiles(eventId);
  ...
  async function replaceFile(
    ...
    return withProgressNotification('replace', async () => {
      const newFile = await apiService.createEventFile(eventId, createData);
