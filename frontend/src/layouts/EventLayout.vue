<template>
  <q-layout view="hHh Lpr fff">
    ...
    -->
    <header class="event-header fixed-top">
      <div class="event-header__inner row items-center no-wrap justify-between">
        <nav class="event-nav row items-center no-wrap">
          <q-btn
            v-if="showBackToEvents"
            :to="{ name: 'events' }"
            class="event-back"
            icon="arrow_back"
            round
            unelevated
            :aria-label="t('back_to_events')"
          >
            <q-tooltip>{{ t('back_to_events') }}</q-tooltip>
          </q-btn>
          <router-link
            to="/"
            class="event-brand row items-center no-wrap"
            :aria-label="t('app_name')"
          >
            <q-avatar
              size="36px"
              class="event-brand__logo"
            >
              <!-- TODO: replace with the brand logo once available -->
              <q-icon name="cabin" />
            </q-avatar>
            <span class="event-brand__name text-weight-medium q-ml-sm">
              {{ t('app_name') }}
            </span>
          </router-link>
        </nav>
    ...
    <q-page-container class="event-page-container">
    ...
    <q-footer class="event-footer bg-surface-container text-on-surface-variant">
      <nav
...
const showBackToEvents = computed<boolean>(() => route.name === 'event');
...
useMeta(() => {
  return {
    title: event.value ? to(event.value.name) : t('events'),
...
.event-toolbar.q-toolbar--floating {
  margin: 0;
}
 ...
 */
.event-page-container :deep(.q-page) {
  padding-top: 4rem;
}
 ...
 */
.event-page-container :deep(.q-page:has(.sv-header)) {
  padding-top: 0;
}

.event-page-container :deep(.sv-header) {
  padding-top: 4rem;
}
