...
describe("/api/v1/camps", () => {
  ...
  describe("GET /api/v1/camps", () => {
    ...
    it.todo("should show all public camps");
  ...
  describe("POST /api/v1/camps", () => {
    it.todo("should respond with `201` status code when user is authenticated");

    it.todo("should respond with `401` status code when unauthenticated");

    it.todo("should respond with `400` status code when body is invalid");
  });
  ...
  describe("DELETE /api/v1/camps/:campId", () => {
    it.only<CampTestContext>("should respond with `204` status code when user is camp manager", async (context) => {
      const { status } = await request(app)
