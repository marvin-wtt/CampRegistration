...
export class EventFilesRouter extends ModuleRouter {
  ...
  protected registerBindings() {
    this.bindModel('file', (req, id) => {
      ...
      return this.fileService.getModelFile('event', event.id, id);
    });
  }
  ...
  protected defineRoutes() {
    ...
    this.router.get(
      '/',
      auth(),
      guard(hasEventPermission('event.files.view')),
      controller(fileController, 'index'),
    );
    this.router.post(
      '/',
      auth(),
      guard(hasEventPermission('event.files.create')),
      multipart('file'),
