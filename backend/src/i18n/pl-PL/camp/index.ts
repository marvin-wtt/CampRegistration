export default {
  email: {
    retentionDue: {
      ...
      preview: 'Dane zgłoszeń wydarzenia {{ camp.name }} wymagają przeglądu',
      text: {
        title: 'Kończy się okres przechowywania',
        information:
          'Informacje o ochronie danych opublikowane dla wydarzenia {{ camp.name }} mówią zgłaszającym się, że ich dane są przechowywane przez {{ months }} miesięcy $t(camp:email.retentionDue.anchor.{{anchor}}). Dla tego wydarzenia okres ten kończy się {{ dueAt }}.',
        action:
