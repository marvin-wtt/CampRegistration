  ...
  email: {
    retentionDue: {
      ...
      text: {
        ...
        information:
          'The privacy information published for {{ camp.name }} tells registrants their data is kept for {{ months }} months $t(camp:email.retentionDue.anchor.{{anchor}}). For this camp that period ends on {{ dueAt }}.',
        action:
          'Please review the camp’s registrations and delete what is no longer needed. You are the one who can tell what still has to be kept.',
        exceptions:
        ...
        noAutomaticDeletion:
          'Nothing has been deleted automatically. This platform never erases camp data on its own — the decision and the action are yours.',
